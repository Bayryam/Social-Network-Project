...
unsigned MyString::toNumber() const
{
	return getNumberFromText(c_str());
}
...
//added
void MyString::saveMyStringToFile(std::ofstream& ofs) const
{
	size_t strLen = length();
	ofs.write((const char*)&strLen, sizeof(strLen));
	ofs.write(c_str(), strLen + 1);
}

void MyString::readMyStringFromFile(std::ifstream& ifs)
{
	size_t len;
	ifs.read((char*)&len, sizeof(len));
	char* data = new char[len + 1];
	ifs.read(data, len + 1);
	*this = std::move(MyString(std::move(data)));
}
 
bool isSubstr(const MyString& pattern, const MyString& text)
{
	return searchInText(text.c_str(), pattern.c_str());
}